...
if (fields.length) {
  Array.prototype.forEach.call(fields, (field) => {
    ...
    const valueMin = input.getAttribute('min')
      ? +input.getAttribute('min')
      : -Infinity;
    const valueMax = input.getAttribute('max')
      ? +input.getAttribute('max')
      : Infinity;
    const valueStep = input.getAttribute('step')
      ? +input.getAttribute('step')
      : 1;

    field.addEventListener('click', (event) => {
      const haveBtnClass = event.target.classList.contains('field-num__btn');

      if (haveBtnClass && !input.getAttribute('disabled')) {
